            ...
            for assoc in assocs:
                ...
                for rule in rules:
                    ...
                    for pattern in patterns:
                        ...
                        print(f"{Colors.YELLOW}TESTING {Colors.WHITE}[{Colors.CYAN}cache size={str(csize).center(6)}{s} block size={str(bsize).center(3)} {s} associativity={str(assoc).center(2)} {s} replacement_rule={str(rule).center(6)} {s} pattern={str(pattern).center(10)}{Colors.WHITE}]: {Colors.GREEN}{str(counter).center(4)}{Colors.RESET}" , end='\r')
                        counter += 1
